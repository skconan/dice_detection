...
def get_region(x, y, radius):
    ...
    # bottom = int(y + dice_size / 2.) + b
    # right = int(x + dice_size / 2.) + b
    # top = int(y - dice_size / 2.) - a
    # left = int(x - dice_size / 2.) - a
    # region.append([top, left, bottom, right])
    # cv.rectangle(img_result, (left - 40, top - 40),
...
def what_is_pattern(mask):
    ...
    for (cnt, hh) in zip(contours, hierarchy):
        x, y, radius = enclosing_circle(cnt)
        index = table_position[x][y]
        if index == -1:
            ct += 1
    ...
    if (radius_min / radius_max) < 0.75 or ct >= 1:
        return [0] * 9
...
def find_dice(mask, circles):
    ...
    for cir in circles:
        # Interger
        x, y, radius = cir
        region_list = get_region(x, y, radius)
        res_mask = mask.copy()
...
def is_point(contour, hierarchy):
    ...
    ###########################################################################
    x, y, radius = ellipse = enclosing_circle(contour)
    area_circle = math.pi * radius * radius
    if area_circle == 0:
        return False
    ###########################################################################
    area_cnt = cv.contourArea(contour)
    area_ratio = area_cnt / area_circle
    ###########################################################################
    '''
    ...
    print('area_ratio   solidity    len_approx')
    print(area_ratio, '  ', solidity, '    ', len_approx)
    ################################ CONDITION ##################################
    ...
    if not len_approx > len_approx_expected:
        ...
        return False
    if not 1.0 >= area_ratio > area_ratio_expected:
...
def enclosing_circle(cnt):
    (x, y), radius = cv.minEnclosingCircle(cnt)
    return int(x), int(y), int(round(radius))
